...
# Do a Packer build
packer build --only=docker ./packer.io.json
...
docker build --rm -t="gameeso_prod" config/dockerfiles/
echo "Final gameeso_prod image built, ready for docker run"
...
docker rmi -f gameeso_staging gameeso gameeso_prod
